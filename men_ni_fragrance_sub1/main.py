...
# Hardcoded subcategory URLs for Men Niche Perfumes
SUBCATEGORY_URLS = [
    "https://www.boutiqaat.com/ar-kw/men/fragrances/niche-perfumes/l/",
]
...
class BoutiqaatDataPipeline:
    ...
    def _upload_product_image(self, product: Dict, category_name: str, subcategory_name: str) -> str:
        try:
            ...
            s3_path = (
                ...
                f"/day={datetime.now().strftime('%d')}/men/niche-perfumes/images/{safe_category}"
            )
    ...
    def _upload_excel_file(self, local_path: str, category_name: str) -> bool:
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}"
                f"/day={datetime.now().strftime('%d')}/men/niche-perfumes/excel-files"
            )
